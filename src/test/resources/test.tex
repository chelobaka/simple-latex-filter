% Command definitions shouldn't generate any content
\newcommand{\ppath}[2][$\;\rightarrow\;$]{
	\def\nextitem{\def\nextitem{#1}}
	\renewcommand*{\do}[1]{\nextitem\textsf{\fcolorbox{light-gray}{light-gray}{##1}}}
	\docsvlist{\#2}
}[foo \emph{bar} baz]
...
Let's try a footnote with a hyperlink inside\footnote{\emph{See:} \href{https://wikipedia.org}{Wikipedia}}\footnotemark
...
\item[☣️] List item text

\item Another list item text

Test [square brackets] text

\chapter[short name]{long name} % Test translatable optional argument